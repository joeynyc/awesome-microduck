...
- [microduck-tricks](https://huggingface.co/langli11/microduck-tricks) - Three skills trained on Hugging Face Jobs and scored on held-out batteries against the shipped policies: a run into a forward roll that hands off from walking 200 times out of 200 where the official policy manages 86 percent, and a cone slalom that clears 8 of 8 gates without a fall where the official policy clears 21 percent. A fourth, a sustained one-blade glide, is reported as not reachable on this robot. *Sim-only.*
